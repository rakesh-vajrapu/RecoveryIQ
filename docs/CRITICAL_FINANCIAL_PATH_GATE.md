...
| **CFP-03** | Execution idempotency / reservation | one durable execution reservation prevents duplicate provider calls for the same logical execution | ISOLATED_LOCAL_VERIFICATION | Stale local execution reservations are detected and reconciled without blind provider replay. See [External Execution Recovery](EXTERNAL_EXECUTION_RECOVERY.md) |
