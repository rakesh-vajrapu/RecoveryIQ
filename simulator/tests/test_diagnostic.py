...
def test_diagnostic_oracle_non_interference():
    ...
    runtimes: list[Any] = [{"template": item, "state": initial_episode_state(item)} for item in templates]
    for _ in range(3):
        ...
        for rt in active:
            ...
            factual_selections.append(decision.selected.candidate.label if decision.selected else None)
            rt["state"] = rt["state"].model_copy(update={"termination": EpisodeTermination.STOP})
            
    oracle_selections: list[Any] = []
    runtimes2: list[Any] = [{"template": item, "state": initial_episode_state(item)} for item in templates]
    ...
    for _ in range(3):
        ...
        for rt in active:
            ...
            offset += len(rt["candidates"])
            decision = engine.decide(rt["state"], scores)
            oracle_selections.append(decision.selected.candidate.label if decision.selected else None)
    ...
    assert factual_selections == oracle_selections, "Oracle execution mutated hidden state impacting subsequent scores!"
