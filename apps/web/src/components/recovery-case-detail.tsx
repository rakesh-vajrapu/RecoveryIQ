...
function CaseContent({ recoveryCase, onRefresh }: { recoveryCase: RecoveryCaseDetail; onRefresh: () => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <article className="surface-panel rounded-2xl p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="eyebrow">Provider outcome</p><h2 className="mt-2 text-lg font-semibold">Verified payment result</h2></div>{latestOutcome?.verified ? <CheckCircle2 className="size-5 text-emerald-500" /> : <AlertTriangle className="size-5 text-muted-foreground" />}</div>{latestOutcome ? <div className="mt-5 grid gap-3 sm:grid-cols-2"><InfoDatum label="Outcome" value={titleCase(latestOutcome.status)} /><InfoDatum label="Verified" value={latestOutcome.verified ? "Yes · provider event" : "No"} /><InfoDatum label="Amount" value={formatMoney(latestOutcome.amount_minor, latestOutcome.currency)} /><InfoDatum label="Occurred" value={formatDate(latestOutcome.occurred_at)} /></div> : <div className="mt-5"><EmptyPanel title="No verified outcome" description="Only an authenticated provider event can create an external outcome." /></div>}</article>
        <article className="surface-panel rounded-2xl p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="eyebrow">Exactly-once attribution</p><h2 className="mt-2 text-lg font-semibold">Recovered revenue</h2></div><CircleDollarSign className="size-5 text-primary" /></div>{recoveryCase.attribution ? <div className="mt-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.065] p-5"><p className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">{formatMoney(recoveryCase.attribution.amount_minor, recoveryCase.attribution.currency)}</p><p className="mt-2 text-xs text-emerald-800/75 dark:text-emerald-200/70">{titleCase(recoveryCase.attribution.attribution_source)} · {formatDate(recoveryCase.attribution.occurred_at)}</p><div className="mt-4 flex items-center gap-2 border-t border-emerald-500/15 pt-4 text-[10px] font-bold tracking-[0.1em] text-emerald-700 uppercase dark:text-emerald-300"><CheckCircle2 className="size-3.5" />Recorded exactly once</div></div> : <div className="mt-5"><EmptyPanel title="No recovery attribution" description="Attribution appears only after a matching, verified payment outcome." /></div>}</article>
      </section>
    </div>
  );
}
