...
function Metadata({ metadata }: { metadata: Record<string, unknown> }) {
  ...
  const hasMoney = 'amount_minor' in metadata && 'currency' in metadata;
  
  return <dl className="mt-4 grid gap-2 sm:grid-cols-2">
    {hasMoney && (
      <div className="rounded-lg bg-muted/50 px-3 py-2 sm:col-span-2">
        <dt className="text-[9px] font-bold tracking-wider text-muted-foreground uppercase">Amount</dt>
        <dd className="mt-1 break-words font-mono text-sm font-semibold">
          {formatMoney(metadata.amount_minor as number, metadata.currency as string)} {String(metadata.currency)}
        </dd>
      </div>
    )}
    {safeEntries.map(([key, value]) => {
      if (hasMoney && key === 'amount_minor') {
        return (
          <div key={key} className="rounded-lg bg-muted/50 px-3 py-2">
            <dt className="text-[9px] font-bold tracking-wider text-muted-foreground uppercase">Raw minor units</dt>
            <dd className="mt-1 break-words font-mono text-[10px]">{String(value)}</dd>
          </div>
        );
      }
      if (hasMoney && key === 'currency') return null;
      return <div key={key} className="rounded-lg bg-muted/50 px-3 py-2"><dt className="text-[9px] font-bold tracking-wider text-muted-foreground uppercase">{titleCase(key)}</dt><dd className="mt-1 break-words font-mono text-[10px]">{String(value)}</dd></div>
    })}
  </dl>;
}
