  ...
  return (
    <>
      ...
      <div className="mt-5">
        ...
        {traces.length > 0 && <div className="grid gap-4 xl:grid-cols-2">{traces.map(({ recoveryCase, decision }) => <article key={decision.id} className="surface-panel interactive-panel rounded-2xl p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-[10px] font-semibold text-muted-foreground">Case {shortId(recoveryCase.id, 12)}</p><h2 className="mt-2 text-base font-semibold">{decision.selected_action ? titleCase(decision.selected_action) : titleCase(decision.kind)}</h2></div><StatusBadge status={recoveryCase.status} /></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><Datum label="Kind" value={titleCase(decision.kind)} /><Datum label="Model" value={decision.model_version} /><Datum label="Policy" value={decision.policy_version} /></div><div className="mt-4 rounded-xl border bg-[var(--surface-soft)] p-4"><p className="text-[10px] font-bold tracking-wider text-muted-foreground uppercase">Reason</p><p className="mt-2 text-xs leading-5">{titleCase(decision.reason)}</p></div><Link href={`/recovery-cases/${recoveryCase.id}`} className="focus-ring group mt-5 flex items-center gap-2 rounded-lg text-xs font-semibold text-primary">Open complete case<ArrowRight className="size-3.5 transition-transform group-hover:translate-x-1" /></Link></article>)}</div>}
      </div>
    </>
  );
}
