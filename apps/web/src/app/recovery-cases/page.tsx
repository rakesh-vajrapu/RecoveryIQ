    ...
    <>
      ...
      {resource.data && (
        <div className="surface-panel overflow-hidden rounded-2xl">
          <div className="grid gap-3 border-b p-4 sm:p-5 xl:grid-cols-[minmax(240px,1fr)_175px_190px_190px_auto]">
          ...
          {filtered.length === 0 ? <div className="p-5"><EmptyPanel title={resource.data.length ? "No cases match these filters" : "No recovery cases yet"} description={resource.data.length ? "Try a different reference, status, or sort option." : "Cases will appear here when the backend records a recoverable payment failure."} /></div> : (
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full min-w-[1420px] text-left">
                <thead className="bg-muted/45 text-[10px] font-bold tracking-[0.12em] text-muted-foreground uppercase"><tr><th className="px-5 py-3.5 sm:px-6">Case</th><th className="px-4 py-3.5">Source</th><th className="px-4 py-3.5">Failure / method</th><th className="px-4 py-3.5">Amount</th><th className="px-4 py-3.5">Status</th><th className="px-4 py-3.5">Decision</th><th className="px-4 py-3.5">Created</th><th className="px-4 py-3.5">Last activity</th><th className="px-5 py-3.5 text-right sm:px-6">Action</th></tr></thead>
                <tbody className="divide-y">{filtered.map((item) => <tr key={item.id} className="group transition-colors hover:bg-muted/35"><td className="px-5 py-4 sm:px-6"><Link href={`/recovery-cases/${item.id}`} className="focus-ring rounded-md font-mono text-xs font-semibold hover:text-primary">{shortId(item.id, 12)}</Link><p className="mt-1 font-mono text-[10px] text-muted-foreground">{shortId(item.correlation_id, 12)}</p></td><td className="px-4 py-4"><SourceBadge source={item.source} /></td><td className="px-4 py-4"><p className="text-xs font-medium">{titleCase(item.failure_type)}</p><p className="mt-1 text-[10px] text-muted-foreground">{titleCase(item.payment_method)}</p></td><td className="px-4 py-4 text-sm font-semibold">{formatMoney(item.amount_minor, item.currency)}</td><td className="px-4 py-4"><StatusBadge status={item.status} /></td><td className="px-4 py-4"><p className="text-xs font-medium">{item.decision_kind ? titleCase(item.decision_kind) : "Not recorded"}</p><p className="mt-1 text-[10px] text-muted-foreground">{item.decision_reason ? titleCase(item.decision_reason) : "Provider lifecycle evidence"}</p></td><td className="px-4 py-4 text-xs text-muted-foreground">{formatDate(item.created_at)}</td><td className="px-4 py-4 text-xs text-muted-foreground">{formatDate(item.last_activity_at)}</td><td className="px-5 py-4 text-right sm:px-6"><Button variant="ghost" size="sm" nativeButton={false} render={<Link href={`/recovery-cases/${item.id}`} />}><Eye className="size-3.5" />Open case</Button></td></tr>)}</tbody>
              </table>
            </div>
            </>
          )}
