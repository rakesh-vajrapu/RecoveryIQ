...
import { getRazorpayStatus } from "@/lib/api";
import { titleCase } from "@/lib/format";

export default function RazorpayIntegrationPage() {
  const load = useCallback((signal: AbortSignal) => getRazorpayStatus(signal), []);
    ...
    <>
      ...
      {resource.data && <div className="space-y-5">
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <IntegrationCard icon={CreditCard} label="API connection" value={resource.data.api_configured ? "Connected" : "Not configured"} ok={resource.data.api_configured} />
          <IntegrationCard icon={Webhook} label="Webhook" value={resource.data.webhook_configured ? "Active" : "Not configured"} ok={resource.data.webhook_configured} />
          <IntegrationCard icon={Link2} label="Payment Link" value={resource.data.capabilities.CREATE_PAYMENT_LINK === "REAL_TEST_EXECUTION" ? "Available" : "Unavailable"} ok={resource.data.capabilities.CREATE_PAYMENT_LINK === "REAL_TEST_EXECUTION"} />
          <IntegrationCard icon={LockKeyhole} label="Live payments" value="Blocked" ok />
        </section>
        <section className="surface-panel overflow-hidden rounded-2xl"><div className="border-b px-5 py-4 sm:px-6"><p className="eyebrow">Execution capability map</p><h2 className="mt-1.5 text-base font-semibold">What each action is allowed to do</h2></div><div className="grid gap-px bg-border sm:grid-cols-2 xl:grid-cols-3">{Object.entries(resource.data.capabilities).map(([action, capability]) => <div key={action} className="bg-card/85 p-4"><p className="font-mono text-[11px] font-semibold">{action}</p><p className="mt-2 text-xs text-muted-foreground">{titleCase(capability)}</p></div>)}</div></section>
