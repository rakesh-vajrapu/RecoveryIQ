...
_ARTIFACT_PATH = Path("artifacts/demo/safety-verification.json")
_POLICY_PATH = Path("artifacts/policy/recoveriq-sequential-v2/recoveriq-sequential-policy-v2.json")
...
async def run_scenarios() -> dict[str, Any]:
    ...
    async with LocalSafetyHarness() as harness:
        ...
        body, signature = sign_webhook(payload)
        await harness.client.post("/webhooks/razorpay", content=body, headers={
            "x-razorpay-signature": signature,
        ...
        output["scenarios"]["concurrent_executor"] = {
            ...
            "measured": {
                ...
                "fake_provider_calls": harness.gateway.create_calls,
                "fake_provider_resources": harness.gateway.create_calls,
        ...
        output["provider"]["razorpay_network_calls"] += harness.gateway.create_calls
    ...
    async with LocalSafetyHarness() as harness:
        ...
        async def fire_c(i: int) -> Response:
            ...
            return await harness.client.post("/webhooks/razorpay", content=b, headers={
        ...
        with harness.sessions() as session:
            ...
            recovered_transitions = 1 if updated_case and updated_case.status == RecoveryCaseStatus.RECOVERED else 0
    ...
    async with LocalSafetyHarness() as harness:
        ...
        output["scenarios"]["sequential_duplicate"] = {
            ...
            "measured": {
                ...
                "second_call_status": "EXISTING_EXECUTION_REUSED" if res2.status_code == 200 and calls_after_1 == calls_after_2 else "FAILED",
