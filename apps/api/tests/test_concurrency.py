...
async def test_webhook_deduplication_race(razorpay_harness: RazorpayHarness) -> None:
    ...
    async def fire() -> Response:
        return await razorpay_harness.client.post("/webhooks/razorpay", content=body, headers=headers)
    ...
    with razorpay_harness.sessions() as session:
        events = session.scalars(select(ExternalWebhookEvent).where(ExternalWebhookEvent.provider_event_id == event_id)).all()
        assert len(events) == 1
...
async def test_execution_idempotency_race(razorpay_harness: RazorpayHarness) -> None:
    ...
    async def fire() -> Response:
        return await razorpay_harness.client.post(f"/api/recovery-cases/{case_id}/test-payment-link")
    ...
    with razorpay_harness.sessions() as session:
        executions = session.scalars(select(ExternalExecution).where(ExternalExecution.recovery_case_id == case.id)).all()
...
async def test_outcome_attribution_race(razorpay_harness: RazorpayHarness) -> None:
    ...
    await razorpay_harness.client.post(
        ...
        headers={"x-razorpay-signature": signature, "x-razorpay-event-id": "ev_Race3Setup", "content-type": "application/json"}
    )
    ...
    with razorpay_harness.sessions() as session:
        execution = session.scalar(select(ExternalExecution).where(ExternalExecution.recovery_case_id == case.id))
    ...
    paid_payload["payload"]["payment_link"]["entity"]["reference_id"] = execution.provider_reference_id
    ...
    with razorpay_harness.sessions() as session:
        outcomes = session.scalars(select(ExternalOutcome).where(ExternalOutcome.recovery_case_id == case.id)).all()
        assert len(outcomes) == 1
        
        attributions = session.scalars(select(RecoveryAttribution).where(RecoveryAttribution.recovery_case_id == case.id)).all()
